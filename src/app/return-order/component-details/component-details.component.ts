...
export class ComponentDetailsComponent implements OnInit, OnDestroy {
  ...
  cancelOrder() {
    ...
    if (confirm('Are you sure you want to cancel!')) {
      if(this.componentDetails?.id)
      {
        this.componentProcessingService.deleteRequest(this.componentDetails.id).subscribe(() => {
      ...
      this.toastr.error("Return Order Request is Cancelled!");
    }
    else {
      this.componentProcessingService.setProcessState();
    }
  ...
  confirmOrder(template: TemplateRef<any>) {
    if (this.paymentDetails.creditLimit >= this.paymentDetails.totalProcessingCharge) {
      this.componentProcessingService.completeProcessing(this.paymentDetails).subscribe((result => {
        if (result) {
          debugger;
          this.componentProcessingService.resetProcessState();
      ...
      this.toastr.error("Credit Limit is less than the total processing charge");
    }
  }
